...
namespace teb_local_planner
{
  ...
  // aligned_allocator相当于管理内存的方式见https://blog.csdn.net/qq_18305555/article/details/87915778
  // 这个可以看成std::vector<Eigen::Vector2d> ViaPointContainer
 ...
 */
   ...
   */
    ...
    /**
   * 构造函数，初始化了cfg参数、机器人模型、viapoint和起点终点速度
   ...
   * 调用此方法来创建和优化根据初始参考计划(作为姿态的容器)初始化的轨迹。
   ...
   * 如果还不存在轨迹，则根据初始计划初始化一个新的轨迹，看到TimedElasticBand:: initTEBtoGoal
   * 如果之前的解决方案是可行的，那么根据最初的计划更新轨迹，
   * 参见bool TimedElasticBand::updateAndPruneTEB然后通过调用optimizeTEB()和调用g20来优化最近初始化或更新的轨迹
   ...
   */
    ...
    //  调用方法去生成和优化轨迹（根据先前产生的轨迹，调用optimizeTEB（）来优化轨迹。
   ...
   */
    ...
    /**
   * 里面有两层循环，分别叫外部循环和内部循环。
   * 外部循环通过调用TimedElasticBand::autoResize()来根据时间分辨率调整轨迹。
   * 内部循环调用optimizeGraph（）进行优化。
   * 外部的循环时间应根据cpu的控制速率要求来定，内部循环的时间经过实验2-6次足够。
   * 最后调用see computeCurrentCost().计算轨迹花费。
   * @brief Optimize a previously initialized trajectory (actual TEB optimization loop).
   * 优化之前初始化的轨迹(实际的TEB优化循环)。
   * 
   ...
   * 内外循环迭代的比率显着定义了轨迹优化的收缩行为和收敛速度。根据我们的经验，2-6次内部循环迭代就足够了。
   ...
   * 外部循环迭代的次数应该通过考虑匹配控制速率所需的最大CPU时间来确定。\ n
   ...
   * 如果计划的轨迹在两个具有相似成本的解(在相同的等价类中!)之间摆动，可以指定一个所需的(初始)转弯方向。
   * 检查参数，以便调整刑罚的权重
   * 初始意味着惩罚只适用于轨迹的前几个姿态。
   ...
   */
    ...
    /**
   * 将图中所有边的error的平方和构成cost，存在支持多条轨迹的重载版本。
   ...
   */
    ...
    /**
   * 获得整个轨迹的位姿、速度、时间。将其存储在TrajectoryPointMsg类型的vector中。
   ...
   */
    ...
    /**
   * 将timedElasticBand类型中的位姿顶点序列、时间顶点序列依次加入图中。note：添加顺序会影响稀疏矩阵的结构进而影响优化效率。
   * @brief Add all relevant vertices to the hyper-graph as optimizable variables.
   * 将所有相关的顶点作为优化变量添加到超图中。
   * 
   ...
   * 顶点插入的顺序对效率很重要，因为它会影响优化计算的底层hessian的稀疏性模式。
   * @see VertexPose
   * @see VertexTimeDiff
   * @see buildGraph
   * @see optimizeGraph
