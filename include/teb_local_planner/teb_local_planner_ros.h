...
namespace teb_local_planner
{
  ...
  */
   ...
   */
    ...
    * 机器人的姿态通过最近的tf变换转换到全局计划的框架中。
    * 如果找不到有效的转换，该方法将返回\c false。
    * 全局计划被修剪，直到与机器人之间的距离至少是dist_behind_robot。
    * 如果在robot后面指定的treshold \c区域内没有找到任何位置，则不会删除任何内容，并且该方法返回\c false
    * 就是机器人前面dist_behind_robot距离的之外的路径都被删减
    ...
    * 返回当前目标姿势的索引以及全局计划和计划框架之间的转换。
    * @param tf A reference to a transform listener
    * @param global_plan The plan to be transformed
    * @param global_pose The global pose of the robot
    * @param costmap A reference to the costmap being used so the window size for transforming can be computed
    *               对正在使用的成本图的引用，因此可以计算用于转换的窗口大小
    ...
    * 指定转换后的计划的最大长度（累积欧几里得距离）[如果<= 0：禁用；长度也受本地costmap大小限制！]
    * @param[out] transformed_plan Populated with the transformed plan 用转换后的路径去填充transformed_plan
    * @param[out] current_goal_idx Index of the current (local) goal pose in the global plan目标姿势的索引
    * @param[out] tf_plan_to_global Transformation between the global plan and the global planning frame全局计划和计划框架之间的转换
   ...
   * 倒车时平动速度的限制可以独立地改变。不要选择max vel x backward <= o。如果不需要反向驱动，则更改优化权重来惩罚反向驱动。
  ...
  private:
    ...
    // 局部轨迹优化时需要考虑的障碍向量 每一个元素存储着是否是动态障碍物和如果是障碍物那么他的速度是多少
    ...
    TebConfig cfg_;                    //!< Config class that stores and manages all related parameters存储和管理所有相关参数的配置类
